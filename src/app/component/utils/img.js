 ...
 * 压缩图片
 */
function compress(file, options, callback, onError) {
    ...
    reader.onload = function (evt) {
        ...
        img.onload = function () {
            ...
            if(!options.type) options.type = 'image/jpeg';
            
            dataURL = canvas.toDataURL(options.type, options.quality);

            if (/;base64,null/.test(dataURL) || /;base64,$/.test(dataURL)) {
                // 压缩失败，以base64上传的，直接报错不上传
                console.error('Compress fail, dataURL is ' + dataURL + '.');
                if (onError) onError(file, new Error('Compress fail, dataURL is ' + dataURL + '.'));
            } else {
                // file.base64 = dataURL;
                callback(dataURL);
            }
